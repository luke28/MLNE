        ...
        for _ in xrange(maxx):
            ...
            for i in xrange(p.batch_size):
                ...
                if u >= base_len and v >= base_len:
                    raise Exception("This edge shouldn't be there")
                ...
                for j in xrange(p.num_sampled):
                    ...
                    if u < base_len:
                        ...
                        v = random.randint(0, base_len - 1)
                        ind = float(sub_params["in_degree"][v])
                        tmp = 1.0 / info["q"][1]
                    tmp *= ind * outd / float(info["total_degree"]) / float(G.number_of_edges())
                    tmp *= float(len(rmapp) ** 2 - (len(rmapp) - base_len) ** 2)
