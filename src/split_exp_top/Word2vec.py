...
def word2vec(walks,vocabulary_size,params,w =None):
    ...
    if vocabulary_size < params['window_size']:
      batch, labels = generate_batch(data,batch_size=params['batch_size'], num_skips=2, skip_window=1)
    else:
    ...
    if vocabulary_size < params['num_sampled']:
      num_sampled = 1
    else:
      ...
      with tf.device('/cpu:0'):
        ...
        with tf.name_scope('embeddings'):
          if w is None:
            ...
            embeddings = tf.Variable(w, dtype = tf.float32)
    ...
    with tf.Session(graph=graph) as session:
        ...
        for step in xrange(num_steps):
            if vocabulary_size < skip_window:
              batch_inputs, batch_labels = generate_batch(data,batch_size, num_skips,1)
            else:
